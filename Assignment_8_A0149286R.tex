...
In the optimal outcome, the items are assigned to the top $k$ player values.
Then, if player $j$ is absent, there exists an optimal outcome $o_{-j}^*$
where every player that was asssigned an item in $o^*$, including $j'$,
is present, except for $j$, who would be replaced by 
the top $(k+1)^{th}$ player value, say player $j''$, for the items.
...
\subsection{}
No. Consider 3 players with valuations
\[v_1=1,v_2=2,v_3=3\]
